...
--[[w = {x=0, y=0, label="skiing"};
print('==============2')
w['1'] = "another filed";
--table.foreach(math,print)
local sin = math.sin;
sin(1)
x = {'1', '2',sin(1)};
x['f'] = w;
...
do
	...
	print(x .. " ");
	i = i + 1;--there is no i++ or ++i in lua
end
print(x);
--]]
--[[
days = {x="Sunday", "Monday", "Tuesday"};
revDay = {};
...
revDay.x = days;
print_lua_table(revDay);
--print(get_msg(revDay));
echo(revDay);
--]]
--[[
local max,index = maxindex({3,-1,30,89,6,34});
print(max .. " " .. index);
--]]
...
function createLayerMy()
	...
	local layer = MySprite:createMySprite();
	
	local _font = CCLabelTTF:create("我的自定义类", "Arial", 33);
	_font:setPosition(230, 230);
	layer:addChild(_font);
	
	layerFarm:addChild(layer);
...
require "MySprite"
local sp = MySprite:createMS("dog.png");
sp:setPosition(100, 100);
--CCMessageBox("Hello", "test");
